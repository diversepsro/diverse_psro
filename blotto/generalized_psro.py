...
class GenPSROSolver(abstract_meta_trainer.AbstractMetaTrainer):
  ...
  def __init__(self,
    ...
    self._lambda_weight = self._num_players*[1.] if lambda_weight is None else lambda_weight
    ...
    for current_player in range(self._num_players):
      ...
      for pol in currently_used_policies:
        new_policy = self._oracle(
            ...
            rectify_training=self._rectify_training[current_player],
            metagame=self._meta_games[current_player],
            lambda_weight=self._lambda_weight[current_player])
